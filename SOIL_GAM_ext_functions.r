...
arm_gam <- function(candidate_mat, L, y, X) {
  ...
  for (l in 1:L) {
    ...
    for (row_idx in 1:nrow(candidate_mat)) {
      ...
      if (row_idx > 1) {
        for (prev_idx in 1:(row_idx - 1)) {
          if (all(candidate_mat[row_idx, ] == candidate_mat[prev_idx, ])) {
            count.vec[row_idx]  <- count.vec[row_idx] + 1
            unique.row[row_idx] <- prev_idx
            break
          }
        }
      }
      
      # If duplicate, reuse previous results
      if (count.vec[row_idx] > 0) {
        sig.hat[row_idx]     <- sig.hat[unique.row[row_idx]]
        df.resid[row_idx]    <- df.resid[unique.row[row_idx]]
        fitted.vals[row_idx, ] <- fitted.vals[unique.row[row_idx], ]
        preds[row_idx, ]     <-  preds[unique.row[row_idx],]
        edf[row_idx]         <-  edf[unique.row[row_idx]]
        C[row_idx]           <-  C[unique.row[row_idx]]
      } 
      else {
        ...
        #edf[row_idx]     <- length(train.index) - df.resid[row_idx]
        #print(max(edf))
